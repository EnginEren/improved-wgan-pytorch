...
class DCGAN_D(nn.Module):
    ...
    def forward(self, x, energy):
        ...
        x = F.leaky_relu(self.conv1(x), 0.2) # 15 x 15
        x = F.leaky_relu(self.conv2(x), 0.2) # 14 x 14
        x = self.lnorm1(x)  ## layer-norm
        x = F.leaky_relu(self.conv3(x), 0.2) # 13 x 13
        x = self.lnorm2(x)  ## layer-norm
        x = F.leaky_relu(self.conv4(x), 0.2)  # 6x6
        x = self.lnorm3(x)  ## layer-norm
...
class DCGAN_G(nn.Module):
    ...
    def forward(self, z):
        ...
        for i in range(self.nc):     
            ...
            x = F.leaky_relu(self.bn1(self.deconv1(z)), 0.2)  # 4 x 4 
            x = F.leaky_relu(self.bn2(self.deconv2(x)), 0.2)  # 8 x 8
            x = F.leaky_relu(self.bn3(self.deconv3(x)), 0.2)  # 16 x 16 
            x = F.leaky_relu(self.bn4(self.deconv4(x)), 0.2)  # 32 x 32
